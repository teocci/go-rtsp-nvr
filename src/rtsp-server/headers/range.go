...
func (t *RangeSMPTETime) read(s string) error {
	...
	if len(parts) != 3 && len(parts) != 4 {
		return liberrors.ErrorInvalidSMPTETime(s)
	}
...
func (t *RangeNPTTime) read(s string) error {
	parts := strings.Split(s, ":")
	if len(parts) > 3 {
		return liberrors.ErrorInvalidNTPTime(s)
	}
...
func rangeValueRead(s RangeValue, v string) error {
	...
	if len(parts) != 2 {
		return liberrors.ErrorInvalidValueWithV(v)
	}
...
func (h *Range) Read(v base.HeaderValue) error {
	...
	if !specFound {
		return liberrors.ErrorValueNotFound(v[0])
	}
