...
func (tm TransportMode) AsInt() int {
	return int(tm)
}
...
func parsePorts(val string) (*[2]int, error) {
	...
	if len(ports) == 2 {
		...
		if err != nil {
			return &[2]int{0, 0}, liberrors.ErrorInvalidPorts(val)
		}
...
func (h *Transport) Read(v base.HeaderValue) error {
	...
	for k, rv := range kvs {
		...
		case "source":
			ip := net.ParseIP(v)
			if ip == nil {
				host, err := net.LookupHost(v)
				if err != nil {
					return liberrors.ErrorInvalidSource(v)
				}
		...
		case "destination":
			ip := net.ParseIP(v)
			if ip == nil {
				return liberrors.ErrorInvalidDestination(v)
			}
		...
		case "ssrc":
			...
			if len(tmp) > 4 {
				return liberrors.ErrorInvalidSSRC()
			}
		...
		case "mode":
			...
			default:
				return liberrors.ErrorInvalidTransportMode(str)
			}
	...
	if !protocolFound {
		return liberrors.ErrorProtocolNotFound(v[0])
	}
