...
func TestInterleavedFrameReadErrors(t *testing.T) {
	for _, ca := range []struct {
		...
		t.Run(ca.name, func(t *testing.T) {
			var f InterleavedFrame
			err := f.Read(5, bufio.NewReader(bytes.NewBuffer(ca.bts)))
...
func TestReadInterleavedFrameOrRequest(t *testing.T) {
	bts := []byte("DESCRIBE rtsp://example.com/media.mp4 RTSP/1.0\r\n" +
		"Accept: application/sdp\r\n" +
		"CSeq: 2\r\n" +
		"\r\n")
	bts = append(bts, []byte{0x24, 0x6, 0x0, 0x4, 0x1, 0x2, 0x3, 0x4}...)

	var f InterleavedFrame
	var req Request
	br := bufio.NewReader(bytes.NewBuffer(bts))
...
func TestReadInterleavedFrameOrRequestErrors(t *testing.T) {
	for _, ca := range []struct {
		...
		t.Run(ca.name, func(t *testing.T) {
			var f InterleavedFrame
			var req Request
			br := bufio.NewReader(bytes.NewBuffer(ca.bts))
