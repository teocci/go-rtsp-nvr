...
func TestResponseRead(t *testing.T) {
	...
	for _, c := range casesResponse {
		t.Run(c.name, func(t *testing.T) {
			err := res.Read(bufio.NewReader(bytes.NewBuffer(c.bts)))
...
func TestResponseReadErrors(t *testing.T) {
	for _, ca := range []struct {
		...
		t.Run(ca.name, func(t *testing.T) {
			var res Response
			err := res.Read(bufio.NewReader(bytes.NewBuffer(ca.bts)))
...
func TestResponseWrite(t *testing.T) {
	for _, c := range casesResponse {
		t.Run(c.name, func(t *testing.T) {
			...
			c.res.Write(&buf)
			require.Equal(t, c.bts, buf.Bytes())
		})
	}
}
...
func TestResponseWriteAutoFillStatus(t *testing.T) {
	...
	bts := []byte("RTSP/1.0 405 Method Not Allowed\r\n" +
		"CSeq: 2\r\n" +
		"Date: Sat, Aug 16 2014 02:22:28 GMT\r\n" +
		"Session: 645252166\r\n" +
		"WWW-Authenticate: Digest realm=\"4419b63f5e51\", nonce=\"8b84a3b789283a8bea8da7fa7d41f08b\", stale=\"FALSE\"\r\n" +
	...
	var buf bytes.Buffer
	res.Write(&buf)
	require.Equal(t, bts, buf.Bytes())
}

func TestResponseReadIgnoreFrames(t *testing.T) {
	bts := []byte{0x24, 0x6, 0x0, 0x4, 0x1, 0x2, 0x3, 0x4}
	bts = append(bts, []byte("RTSP/1.0 200 OK\r\n"+
		"CSeq: 1\r\n"+
		"Public: DESCRIBE, SETUP, TEARDOWN, PLAY, PAUSE\r\n"+
		"\r\n")...)

	rb := bufio.NewReader(bytes.NewBuffer(bts))
	var res Response
...
func TestResponseString(t *testing.T) {
	...
	var res Response
	err := res.Read(bufio.NewReader(bytes.NewBuffer(bts)))
	require.NoError(t, err)
	require.Equal(t, string(bts), res.String())
}
