...
import (
	...
	"github.com/teocci/go-rtsp-nvr/src/rtsp-server/auth"
...
type Client struct {
	...
	// called when RTP packets arrive.
	OnPacketRTP func(*ClientOnPacketRTPCtx)
	// called when RTCP packets arrive.
	...
	// read buffer count.
	// If greater than 1, allows passing buffers to other routines
...
func (c *Client) Start(scheme string, host string) error {
	...
	if c.ReadBufferCount == 0 {
		c.ReadBufferCount = defaultBufferCount
	}
	if c.WriteBufferCount == 0 {
		c.WriteBufferCount = defaultBufferCount
	}
	...
	if c.udpSenderReportPeriod == 0 {
		c.udpSenderReportPeriod = defaultTimeout
	}
	if c.udpReceiverReportPeriod == 0 {
		c.udpReceiverReportPeriod = defaultTimeout
	}
	if c.checkStreamPeriod == 0 {
		c.checkStreamPeriod = defaultStreamPeriod
	}
...
func (c *Client) runInner() error {
	for {
			...
			if *c.effectiveTransport == TransportUDP ||
				*c.effectiveTransport == TransportUDPMulticast {
				if c.checkStreamInitial {
					...
					if inTimeout {
						return liberrors.ErrorUDPTimeout()
					}
				}
			} else { // TCP
				inTimeout := func() bool {
				...
				if inTimeout {
					return liberrors.ErrorTCPTimeout()
				}
			}
		...
		case <-c.ctx.Done():
			return liberrors.ErrorClientTerminated()
		}
	}
}
...
func (c *Client) checkState(allowed map[clientState]struct{}) error {
	...
	return liberrors.ErrorInvalidState(allowedList, c.state)
}
...
func (c *Client) do(req *base.Request, skipResponse bool, allowFrames bool) (*base.Response, error) {
	...
	if !skipResponse {
		...
		if v, ok := res.Header["Session"]; ok {
			...
			err := sx.Read(v)
			if err != nil {
				return nil, liberrors.ErrorSessionHeaderInvalid(err)
			}
...
func (c *Client) doOptions(u *base.URL) (*base.Response, error) {
	...
	if res.StatusCode != base.StatusOK {
		...
		return nil, liberrors.ErrorBadStatusCode(int(res.StatusCode), res.StatusMessage)
	}
...
func (c *Client) Options(u *base.URL) (*base.Response, error) {
	...
	case <-c.ctx.Done():
		return nil, liberrors.ErrorClientTerminated()
	}
}
...
func (c *Client) doDescribe(u *base.URL) (Tracks, *base.URL, *base.Response, error) {
	...
	if res.StatusCode != base.StatusOK {
		...
		return nil, nil, res, liberrors.ErrorBadStatusCode(int(res.StatusCode), res.StatusMessage)
	}

	ct, ok := res.Header["Content-Type"]
	if !ok || len(ct) != 1 {
		return nil, nil, nil, liberrors.ErrorContentTypeMissing()
	}
	...
	if ct[0] != "application/sdp" {
		return nil, nil, nil, liberrors.ErrorContentTypeUnsupported(ct[0])
	}
...
func (c *Client) Describe(u *base.URL) (Tracks, *base.URL, *base.Response, error) {
	...
	case <-c.ctx.Done():
		return nil, nil, nil, liberrors.ErrorClientTerminated()
	}
}
...
// Announce writes ANNOUNCE requests and reads responses.
...
func (c *Client) doSetup(forPlay bool, track Track, baseURL *base.URL, rtpPort int, rtcpPort int) (*base.Response, error) {
	err := c.checkState(map[clientState]struct{}{
		clientStateInitial:   {},
		clientStatePrePlay:   {},
		clientStatePreRecord: {},
	...
	if (!forPlay && c.state != clientStatePreRecord) ||
		(forPlay && c.state != clientStatePrePlay &&
			c.state != clientStateInitial) {
		return nil, liberrors.ErrorCannotReadPublishAtSameTime()
	}

	if c.streamBaseURL != nil && *baseURL != *c.streamBaseURL {
		return nil, liberrors.ErrorCannotSetupTracksDifferentURLs()
	}
	...
	case TransportUDP:
		if (rtpPort == 0 && rtcpPort != 0) ||
			...
			return nil, liberrors.ErrorUDPPortsZero()
		}

		if rtpPort != 0 && rtcpPort != (rtpPort+1) {
			return nil, liberrors.ErrorUDPPortsNotConsecutive()
		}
	...
	if err != nil {
		...
		return nil, liberrors.ErrorTransportHeaderInvalid(err)
	}

	switch transport {
	case TransportUDP:
		if thRes.Delivery != nil && *thRes.Delivery != headers.TransportDeliveryUnicast {
			return nil, liberrors.ErrorTransportHeaderInvalidDelivery()
		}
		...
		if !forPlay || !c.AnyPortEnable {
			if thRes.ServerPorts == nil || isAnyPort(thRes.ServerPorts[0]) || isAnyPort(thRes.ServerPorts[1]) {
				rtpListener.close()
				rtcpListener.close()
				return nil, liberrors.ErrorServerPortsNotProvided()
			}
		}
	...
	case TransportUDPMulticast:
		...
		if thRes.Ports == nil {
			return nil, liberrors.ErrorTransportHeaderNoPorts()
		}

		if thRes.Destination == nil {
			return nil, liberrors.ErrorTransportHeaderNoDestination()
		}
	...
	case TransportTCP:
		...
		if thRes.InterleavedIDs == nil {
			return nil, liberrors.ErrorTransportHeaderNoInterleavedIDs()
		}

		if (thRes.InterleavedIDs[0]%2) != 0 ||
			(thRes.InterleavedIDs[0]+1) != thRes.InterleavedIDs[1] {
			return nil, liberrors.ErrorTransportHeaderInvalidInterleavedIDs()
		}
...
func (c *Client) doPlay(ra *headers.Range, isSwitchingProtocol bool) (*base.Response, error) {
	...
	if *c.effectiveTransport == TransportUDP || *c.effectiveTransport == TransportUDPMulticast {
		for _, cct := range c.tracks {
			bts, _ := (&rtp.Packet{Header: rtp.Header{Version: 2}}).Marshal()
			cct.udpRTPListener.write(bts)

			bts, _ = (&rtcp.ReceiverReport{}).Marshal()
			cct.udpRTCPListener.write(bts)
		}
	}
...
func (c *Client) WritePacketRTP(trackID int, pkt *rtp.Packet, ptsEqualsDTS bool) error {
	...
	bts := make([]byte, maxPacketSize)
	n, err := pkt.MarshalTo(bts)
	if err != nil {
		return err
	}
	bts = bts[:n]
